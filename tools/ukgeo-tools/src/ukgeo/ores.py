...
def make_ore_tiles(*, bgs: Path, rules: Path, manifest_path: Path, out: Path, debug_geotiff_dir: Path | None = None) -> None:
    ...
    try:
        for ore in layer_names:
            layer_config = configured_layers.get(ore, {})
            ...
            for layer_name in layer_config.get("layers", []):
                ...
                if frame.empty:
                    continue
                if frame.crs and str(frame.crs).upper() not in {"EPSG:27700", "EPSG:27700"}:
            ...
            _write_tiles(arr, out / "ores" / ore, tile_size)
            if debug_geotiff_dir:
                _write_debug(debug_geotiff_dir / f"{ore}.tif", arr, transform)
