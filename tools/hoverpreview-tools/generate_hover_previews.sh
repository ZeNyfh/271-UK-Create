...
PY
  ...
  if is_truthy "$INCLUDE_IRELAND"; then
    if is_truthy "$USE_LEGACY_OSNI_HEIGHT"; then
      ...
      if [[ -n "$COP30_DEBUG_TARGET_MASK_GEOTIFF" ]]; then
        cop30_args+=(--debug-target-mask-geotiff "$COP30_DEBUG_TARGET_MASK_GEOTIFF")
      fi
      if [[ -n "$COP30_DEBUG_LAND_MASK_GEOTIFF" ]]; then
        cop30_args+=(--debug-land-mask-geotiff "$COP30_DEBUG_LAND_MASK_GEOTIFF")
      fi
